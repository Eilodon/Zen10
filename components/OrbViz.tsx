...
// --- Performance Monitor ---
const QualityController = ({ onDowngrade }: { onDowngrade: () => void }) => {
  const { gl } = useThree();
  ...
  useFrame(() => {
    ...
    if (time >= lastTime.current + 1000) {
      ...
      if (fps < 30) {
        ...
        lowFpsCount.current = 0; // Reset if healthy
      }

      // If FPS is low for 3 consecutive checks, downgrade
      if (lowFpsCount.current >= 3) {
        onDowngrade();
        lowFpsCount.current = 0; // Prevent repeated downgrades
      }
...
const QuantumOrb: React.FC<OrbProps & { detail: number, isLowPower: boolean }> = ({ analyser, emotion, frequencyData, detail, isLowPower }) => {
  ...
  useFrame((state) => {
    ...
    if (frequencyData && frequencyData.length >= 24) {
      ...
      // OPTIMIZATION: Skip vertex manipulation on low power/mobile devices
      if (!isLowPower && meshRef.current.geometry) {
...
export default function OrbViz({ analyser, emotion, frequencyData }: OrbProps) {
  ...
  const [isLowPower, setIsLowPower] = useState(initialLowPower || isMobile);
  const [isVisible, setIsVisible] = useState(true);

  // VISIBILITY API: Pause rendering when tab is hidden to save battery
  useEffect(() => {
  ...
  // ADAPTIVE QUALITY: Downgrade if FPS < 30
  const handleDowngrade = () => {
    if (!isLowPower) {
      console.log("[Performance] Downgrading visual quality due to low FPS");
      setIsLowPower(true);
    }
  };
  ...
  return (
    <div className="w-full h-full absolute inset-0 z-0 pointer-events-none">
      <Canvas 
        dpr={isLowPower ? 1 : [1, 2]} // Force dpr 1 on low power
        frameloop={isVisible ? 'always' : 'never'} // Pause when hidden
        camera={{ position: [0, 0, 6], fov: 45 }}
        gl={{ 
          antialias: !isLowPower, // Disable AA on low-power
          powerPreference: 'high-performance',
          alpha: true
        }}
      >
        <QualityController onDowngrade={handleDowngrade} />
        ...
        <QuantumOrb 
          ...
          detail={isLowPower ? 0 : 4} // Low poly on low power
          isLowPower={isLowPower} 
        />
        
        {/* Disable particles completely on low power mode */}
        {!isLowPower && (
          <Particles 
            color={emotion} 
            frequencyData={frequencyData}
