...
export const playEmergencyAlert = async () => {
  await getSharedAudioContext(); // Ensure context is ready
  await Tone.start();
...
export const AudioEngine: React.FC<Props> = ({ emotion, breathing, ambientSound, isSpeaking }) => {
  ...
  useEffect(() => {
    return () => {
      ...
      Object.values(nodes.current).forEach(node => {
          if (node) {
              try { (node as any).dispose(); } catch(e) { console.warn("Cleanup error:", e); }
          }
      });
  ...
  // Initialize Tone Master Chain with Shared Context
  const initAudio = async () => {
    if (isReady) return;
    
    // Ensure we are using the Shared Context
    const ctx = await getSharedAudioContext();
    await Tone.start(); // This resumes the shared context if suspended
  ...
  useEffect(() => {
    const cleanupAmbient = () => {
      if (nodes.current.ambient) {
        ...
        } catch(e) { console.warn("Ambient cleanup:", e); }
  ...
  useEffect(() => {
    const cleanupBreathing = () => {
      if (nodes.current.breathingLoop) {
        try { nodes.current.breathingLoop.stop(); nodes.current.breathingLoop.dispose(); } catch(e) { console.warn("Breathing loop cleanup:", e); }
        nodes.current.breathingLoop = null;
      }
      if (nodes.current.breathingOsc) {
        try { nodes.current.breathingOsc.stop(); nodes.current.breathingOsc.dispose(); } catch(e) { console.warn("Breathing osc cleanup:", e); }
